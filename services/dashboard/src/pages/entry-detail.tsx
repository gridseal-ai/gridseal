    ...
    <div className="space-y-6">
      ...
      {status === "success" && data && (
        <div className="space-y-6">
          ...
          {data.entry.sessionId && chainId && (
            <Link
              to={`/chains/${chainId}/sessions/${data.entry.sessionId}`}
              className="inline-block px-3 py-1.5 text-sm bg-slate-800 border border-slate-700 rounded text-sky-400 hover:bg-slate-700 hover:text-sky-300 transition-colors"
            >
