...
describe("data integrity", () => {
  ...
  it("preserves null fields correctly through serialization round-trip", async () => {
    const chain = createChain("chain-1");
    const result = appendEntry(chain, makeInput({ entryId: "null-test" }));
    if (!result.ok) throw new Error("append failed");
    const entry = result.value.entry;

    await adapter.putEntry(entry);
    const getResult = await adapter.getEntry("null-test");

    expect(getResult.ok).toBe(true);
    if (!getResult.ok) return;
    expect(getResult.value.modelId).toBeNull();
    expect(getResult.value.modelProvider).toBeNull();
    expect(getResult.value.inputHash).toBeNull();
    expect(getResult.value.outputHash).toBeNull();
    expect(getResult.value.inputTokenCount).toBeNull();
    expect(getResult.value.outputTokenCount).toBeNull();
    expect(getResult.value.decisionType).toBeNull();
    expect(getResult.value.confidenceScore).toBeNull();
    expect(getResult.value.sessionId).toBeNull();
    expect(getResult.value.actorId).toBeNull();
    expect(getResult.value.annotation).toBeNull();
    expect(getResult.value.parentEntryId).toBeNull();
  });

  it("preserves empty arrays and objects through serialization", async () => {
    const chain = createChain("chain-1");
    const result = appendEntry(chain, makeInput({ entryId: "empty-test" }));
    if (!result.ok) throw new Error("append failed");
    const entry = result.value.entry;

    await adapter.putEntry(entry);
    const getResult = await adapter.getEntry("empty-test");

    expect(getResult.ok).toBe(true);
    if (!getResult.ok) return;
    expect(getResult.value.policyIds).toEqual([]);
    expect(getResult.value.tags).toEqual({});
    expect(getResult.value.complianceMetadata).toEqual({});
  });
});
